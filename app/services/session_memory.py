...
class SessionMemory:
    def __init__(self, session, db):
        ...
        self.region = getattr(session.user, "region", None) if hasattr(session, "user") and session.user and session.user.region else ""
        self.mood = getattr(session, "exit_mood", None)
        # SAFE tone read (meta_data can be None)
        self.tone = (getattr(session, "meta_data", {}) or {}).get("tone")
        self.genre = session.genre[-1] if session.genre else None
        self.platform = session.platform_preference[-1] if session.platform_preference else None
        self.story_preference = getattr(session, "story_preference", None)
        ...
        self.rejections = [get_game_title_by_id(game_id, db) for game_id in rejected_ids]
        # Remove "Unknown" and de-dup
        seen = set()
        self.rejections = [t for t in self.rejections if t and t != "Unknown" and (t not in seen and not seen.add(t))]

        # Recommended game ids (this session) -> titles
        rec_ids = [rec.game_id for rec in db.query(GameRecommendation).filter(
            GameRecommendation.session_id == session.session_id
        ).all()]
        self.rec_ids = rec_ids
        self.recommended_game = [get_game_title_by_id(game_id, db) for game_id in rec_ids]

        self.likes = getattr(session, "liked_games", []) if hasattr(session, "liked_games") else []
        # Don’t list a game as both liked and rejected
        if self.likes:
        ...
        # Keep full history in memory, but we’ll output only last 10
        self.history = [(i.sender.name, i.content, getattr(i, "tone_tag", None)) for i in getattr(session, "interactions", [])]
    ...
    def to_prompt(self):
        ...
        # Tone/Mood line (avoid printing 'None')
        if self.tone and self.mood:
            out.append(f"The user's tone is '{self.tone}' and mood is '{self.mood}'")
        elif self.tone:
            out.append(f"The user's tone is '{self.tone}'")
        elif self.mood:
            out.append(f"The user's mood is '{self.mood}'")
        ...
        if self.rejections:
            out.append("User rejected these games: " + ", ".join(self.rejections))
        if self.likes:
            out.append("User Liked games: " + ", ".join(self.likes))
        if self.last_game:
            out.append(f"Last game suggested by thrum: {self.last_game}")
        if self.recommended_game:
            out.append("Thrum already suggested/Recommended " + ", ".join(self.recommended_game) + " games till now to user.")

        # Last 10 messages in recent chat
        if self.history:
            last_few = self.history[-10:]
            hist_str = " | ".join([f"{s} says {c}" + (f" .. in tone - {t}" if t else "") for s, c, t in last_few])
            out.append(f"Recent chat: {hist_str}")
